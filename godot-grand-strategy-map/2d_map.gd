...
#
		...
		#states[convert_to_index(map_color)] = province_id;
		#
		#var uv = Vector2(float(int(province_id) % DIM), floor(province_id / DIM))
		#var color := Color(float(int(province_id) % DIM) / (DIM - 1), floor(province_id / DIM) / (DIM - 1), 0.0)
	...
	#if !FileAccess.file_exists("res://Assets/provinces.png"): return false;
	#
	#var image := preload("res://Assets/provinces.png").get_image();
...
#
	...
	#look_up_image = create_lookup_image(result, image_dimensions, "res://Assets/lookup.png")
...
#
	...
	#rendering_device.compute_list_dispatch(compute_list, 256, 256, 1)
...
#func _on_control_gui_input(event):
		...
		#if !mouse_event.is_pressed(): return
#
		#var state_color = look_up_image.get_pixelv((mouse_event.position - map.global_position) / map.global_scale)
		#var state_id  = state_uvs.get(state_color)
		#if state_id:
			#label.set_text(str( state_id))
			#print(state_color, state_uvs)
			#map.material.set_shader_parameter('selected_area', state_color)
