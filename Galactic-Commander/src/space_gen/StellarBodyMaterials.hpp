	...
	private:
		...
		void set_body_and_atmosphere_parms(StellarBodyMaterial& mats, Dictionary body_params, Dictionary atmosphere_params) {
			...
			for (int i = 0; i < body_params.size(); i++) {
			    ...
			    if (key == "noise" or key == "body_texture") { // Noise has to be loaded as a resource from a string as it isn't serializable, this is probably fuckin dumb and there's likely a better way to do this, oh well.
