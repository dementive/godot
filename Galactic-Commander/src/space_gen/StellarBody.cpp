...
void StellarBody::create_body(StellarBodyType body_type, float distance_from_orbit_origin, StellarBodyMaterial materials, Vector3 body_scale, String body_name, bool atmosphere) {
	// This code should be in the constructor but this isn't possible currently https://github.com/godotengine/godot-cpp/issues/953
	set_name(body_name);
	if (body_type == STAR) {
...
void StellarBody::_input_event(Camera3D *camera, const Ref<InputEvent> &event, const Vector3 &position, const Vector3 &normal, int32_t shape_idx){
	...
	if (event->is_class("InputEventMouseButton")) {
		...
		if (input_event_mouse_button->is_pressed() && input_event_mouse_button->get_button_index() == MOUSE_BUTTON_LEFT) {
			...
			window->get_node<Label>("PlanetNameLabel")->get_node<Control>("HUD")->get_node<Panel>("PlanetInfoPanel")->set_visible(true);

			Label *label = window->get_node<Label>("PlanetNameLabel")->get_node<Control>("HUD")->get_node<Panel>("PlanetInfoPanel")->get_node<Label>("PlanetNameLabel");
...
void StellarBody::_mouse_exit() {
	...
	SceneTree* tree = get_tree();
	Window* window = tree->get_root();
	window->get_node<Label>("PlanetNameLabel")->get_node<Control>("HUD")->get_node<Panel>("PlanetInfoPanel")->set_visible(false);
}
...
void StellarBody::generate_body(float distance_from_star, StellarBodyMaterial materials, Vector3 planet_scale, bool has_atmosphere) {
