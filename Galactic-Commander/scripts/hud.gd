extends Control
#
#const Planet = preload("./space_gen/planet.gd")
##const WaypointHUD = preload("../waypoints/waypoint_hud.gd")
#const Util = preload("./util.gd")
#
#@onready var _solar_system : SolarSystem = get_parent().solar_system
...
#var _pointed_body : StaticBody3D = null
#
#
##func _ready():
...
#func _process(_delta: float):
	...
	#else:
		#var body_pos : Vector3 = pointed_body.node.global_transform.origin
		#var right := camera.global_transform.basis.x
		#var body_edge_pos : Vector3 = body_pos + right * pointed_body.radius
...
#
#
#func _get_stellar_body_type_name(body: StaticBody3D) -> String:
	##if body.type == StellarBody.TYPE_SUN:
		##return "Star"
	##var parent_body : Planet = _solar_system.get_stellar_body(body.parent_id)
	##if parent_body.type != StellarBody.TYPE_SUN:
		##return "Moon"
	#return "Planet"
#
#
#func _find_pointed_planet(camera: Camera3D) -> StaticBody3D:
	#var camera_pos := camera.global_transform.origin
	#var mouse_pos := get_viewport().get_mouse_position()
	#var ray_origin := camera.project_ray_origin(mouse_pos)
	#var ray_normal := camera.project_ray_normal(mouse_pos)
	#var pointed_body : StaticBody3D = null
	#var closest_distance_squared := -1.0
	#for i in _solar_system.get_stellar_body_count():
		#var body : StaticBody3D = _solar_system.get_stellar_body(i)
	...
	##print(_solar_system.get_stellar_body_count())
	##print(pointed_body)
	#return pointed_body
#
