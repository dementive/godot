...
# front = (0,0,-1)
# back = (0,0,1)
# TODO - don't export this and set each face's normal with code instead.
...
func regenerate_mesh(planet_data: PlanetData):
	...
	for y in range(resolution):
		for x in range(resolution):
			...
			var length = pointOnPlanet.length()
			if length < planet_data.min_height:
				planet_data.min_height = length
			if length > planet_data.max_height:
				planet_data.max_height = length
